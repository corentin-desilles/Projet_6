const http = require('http'); // remplacer par httpS
...
const normalizePort = val => {      //renvoie un port valide, qu'il soit fourni sous la forme d'un numéro ou d'une chaîne ;
  ...
  const port = normalizePort(process.env.PORT || '3000');// indiquer à l'app express sur quel port elle va tourner
  app.set('port', port);
...
const server = http.createServer(app); //methode qui prend comme argument la fonction qui sera appelée à chaque requête reçue par le serveur
